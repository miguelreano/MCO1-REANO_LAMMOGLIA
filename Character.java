...
public class Character {
    ...
    public void startCharacterCreation() {
        ...
        while (!isConfirmed) {
            ...
            switch (choice) {
                ...
                case 3:
                    if (characterName.isEmpty() || characterStats == null) {
                        System.out.println("Please ensure you have chosen a name and class.");
                    } else {
                        System.out.println("Confirm Character?");
                        System.out.println("[YES] [NO]");
                        String confirm = scanner.nextLine();
                        if (confirm.equalsIgnoreCase("yes")) {
                            isConfirmed = true;
                            // Assuming Menus.menusGameLobby(characterStats) is a static method call
                            // to transition to the game lobby after character creation
                            Menus.menusGameLobby(characterStats); // Modified to fit the context
                        } else if (confirm.equalsIgnoreCase("no")) {
                            System.out.println("Character creation not confirmed, returning to options.");
                        }
                    }
                    break;
                case 4:
                    // Assuming Menus.TitleScreen() is a static method to show the title screen
                    Menus.TitleScreen(); // Placeholder for the actual implementation
                    break;
                default:
                    System.out.println("Invalid Input!");
                    break; // Modified to avoid recursive call
            }
        }
    }
    ...
    class CharacterStats {
        ...
        public int addRunes(int amount) {
            return this.Runes += amount;
        }

        public int subtractRunes(int amount) {
            return this.Runes -= amount;
        }
    }
